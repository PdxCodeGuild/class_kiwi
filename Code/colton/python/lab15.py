...
def random_quote():
    ...
    author_quote = {
        "Author": quote_json["quote"]["author"],
        "Quote": quote_json["quote"]["body"],
    }
...
def find_quote():
    ...
    while True:
        authors = requests.get(
            f"https://favqs.com/api/typeahead",
            headers={
        ...
        authorlist = authors.json()

        for index in authorlist["authors"]:
            print(index["name"])

        user_keyword = input(
            "Enter an author to recieve a quote, or 'exit' to leave program\n"
        )
        ...
        if user_keyword == "exit":
            ...
            try:
                ...
                response = requests.get(
                    f"https://favqs.com/api/quotes/?filter={keyword_form}&type=author",
                    headers={
                        "Accept": "application/json",
                        "Authorization": 'Token token="855df50978dc9afd6bf86579913c9f8b"',
                    },
                ...
                while True:
                    try:
                        ...
                        answer = (
                            input(
                                f"Would you like to here another quote from {user_keyword}? Yes or No.\n"
                            )
                            .lower()
                            .strip()
                        )
